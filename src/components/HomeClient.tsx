...
export function HomeClient({ books, plans }: { books: Book[]; plans: Plan[] }) {
      ...
      <section className="page-pad mx-auto grid max-w-6xl items-center gap-8 py-10 md:grid-cols-2 md:gap-10 md:py-16">
        <div className="min-w-0">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link
              href="/books"
              className="rounded-full border border-ink/20 px-6 py-3 text-sm hover:border-wine hover:text-wine"
            >
              {t.cta.browse}
            </Link>
          </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-5 md:grid-cols-4">
          {featured.map((book) => (
            <Link key={book.id} href={`/books/${book.id}`} className="group">
              ...
              <p className="mt-3 font-display text-lg leading-snug group-hover:text-wine">
                {book.title[lang]}
              </p>
