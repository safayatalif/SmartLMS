...
import type { Book, Member, Plan } from "@/lib/types";
...
export function AdminOverviewClient({
  ...
  stats: { members: number; books: number; borrowed: number; revenue: number };
  members: Member[];
  books: Book[];
  plans: Plan[];
}) {
  ...
  const cards = [
    { label: t.admin.membersLabel, value: stats.members.toLocaleString(), hint: "Registered members" },
    { label: t.admin.titlesLabel, value: stats.books.toLocaleString(), hint: "Titles in catalog" },
    { label: t.admin.borrowedLabel, value: stats.borrowed.toLocaleString(), hint: "Copies on loan" },
    {
  ...
  const popular = [...books].sort((a, b) => b.borrowed - a.borrowed).slice(0, 5);
    ...
    <div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {cards.map((card) => (
          <article
            key={card.label}
            className="rounded-2xl border border-ink/10 bg-card p-5 shadow-[0_8px_24px_rgba(28,22,18,0.04)]"
          >
            ...
            <tbody>
              {members.slice(0, 5).map((m) => (
                <tr key={m.id} className="border-t border-ink/8">
                  ...
                  <td className="hidden py-3 md:table-cell">{planLabel(plans, m.plan, lang)}</td>
                  <td className="py-3">
                    <StatusChip
                      tone={m.status === "active" ? "ok" : m.status === "pending" ? "warn" : "muted"}
                    >
                      {m.status === "active"
                        ? t.admin.active
                        : m.status === "pending"
              ...
              <li key={book.id} className="flex items-center justify-between gap-3">
                ...
                <span className="h-1.5 w-20 shrink-0 overflow-hidden rounded-full bg-paper-2">
                  <span
                    ...
                    style={{
                      width: `${book.copies ? Math.round((book.borrowed / book.copies) * 100) : 0}%`,
                    }}
                  />
                </span>
              </li>
